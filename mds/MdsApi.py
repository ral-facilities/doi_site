...
class MdsApi:
    ...
    An instance of the class must first be created, passing the DJango request
    ...
    def _authorise(self, url):
        ...
        if path.startswith("/doi/"):
            ...
            raise MdsApiError(
                "URL path must start with one of ['/doi/', '/metadata/', '/media/']"
            )
    ...
    def _do_request(self, method, url, **kwargs):
        ...
        auth_string = (
            (
                base64.encodebytes(
                    (DATACITE_USER_NAME + ":" + DATACITE_PASSWORD).encode()
                )
            )
            .decode("utf-8")
            .rstrip()
