...
class TestDictToXml(unittest.TestCase):
    def test_mandatory(self):
        ...
        self.assertIn(
            '<resourceType resourceTypeGeneral="Text">Text</resourceType>', res
        )
        self.assertIn(
            '<creator><creatorName nameType="Personal">CreatorFName, CreatorGName</creatorName><givenName>CreatorGName</givenName><familyName>CreatorFName</familyName></creator>',
            res,
        )
    ...
    def test_missing_orcid(self):
        ...
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<nameIdentifier schemeURI="https://orcid.org/" nameIdentifierScheme="ORCID">CREATORORCID</nameIdentifier>',
            res,
        )
    ...
    def test_missing_description(self):
        ...
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<descriptions><description descriptionType="Abstract">ABSTRACT</description></descriptions>',
            res,
        )
    ...
    def test_missing_resource_type_text(self):
        ...
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<resourceType resourceTypeGeneral="Text">RESOURCE TYPE TEXT</resourceType>',
            res,
        )
    ...
    def test_missing_funder_identifier(self):
        ...
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<funderIdentifier funderIdentifierType="Crossref Funder ID">999</funderIdentifier>',
            res,
        )
    ...
    def test_missing_award_number(self):
        dictcopy = copy.deepcopy(dict)
        dictcopy["funders"] = [
            {"funder_name": "", "funder_identifier": "", "award_number": "666"}
        ]
        res = dict_to_xml(dictcopy)
    ...
    def test_missing_award_title(self):
        dictcopy = copy.deepcopy(dict)
        dictcopy["funders"] = [
            {"funder_name": "", "funder_identifier": "", "award_title": "AWARD TITLE"}
        ]
        res = dict_to_xml(dictcopy)
        ...
        dictcopy["dates"] = [
            {
                "date": "2022-02-22",
                "date_type": "Updated",
                "date_text": "Updated with 4.4 properties",
            }
        ]
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<dates><date dateType="Updated" dateInformation="Updated with 4.4 properties">2022-02-22</date></dates>',
            res,
        )

    def test_missing_related_identifier(self):
        dictcopy = copy.deepcopy(dict)
        dictcopy["related_identifiers"] = [
            {
                "related_identifier": "www.site-url.com",
                "related_identifier_type": "URL",
                "related_identifier_relation_type": "Cites",
            }
        ]
        res = dict_to_xml(dictcopy)
        self.assertIn(
            '<relatedIdentifier relatedIdentifierType="URL" relationType="Cites">www.site-url.com</relatedIdentifier>',
            res,
        )
