...
class DoiForm(forms.Form):
    ...
    resource_type_text = forms.CharField(
        label="Resource type:",
        required=False,
        widget=forms.TextInput(
            attrs={
                "placeholder": "Resource type other",
                "class": "form-control form-control-sm",
                "id": "resourceType",
            }
        ),
    ...
    def clean(self):
        ...
        if identifier in self._domains:
            self._errors["identifier"] = self.error_class(
                [
                    "Cannot use Sub-Domain by itself, please append with unique identifiying data"
                ]
            )
        elif r.status_code == 200:
            self._errors["identifier"] = self.error_class(
                ["DOI is already registered, please use another"]
            )
    ...
    related_identifier = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                ...
                "class": "related-identifier form-control form-control-sm",
                "id": "relatedIdentifier",
            }
        ),
    )
    related_identifier_type = forms.CharField(
        label="Pick the identifier type:",
        required=False,
        widget=forms.Select(
            choices=RIT_CHOICES,
            attrs={
                "class": "related-identifier-type form-select form-select-sm",
                "id": "relatedIdentifier",
            },
        ),
    )
    related_identifier_relation_type = forms.CharField(
        label="Pick the relation type:",
        required=False,
        widget=forms.Select(
            choices=RIRT_CHOICES,
            attrs={
                "class": "related-identifier-relation-type form-select form-select-sm",
                "id": "relatedIdentifier",
            },
        ),
    )
    ...
    funder_identifier = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={
                ...
                "class": "form-control form-control-sm funder_id",
                "id": "funder",
            }
        ),
    )
...
class DateForm(forms.Form):
    ...
    date_type = forms.CharField(
        label="Pick the date type:",
        required=False,
        widget=forms.Select(
            choices=DT_CHOICES,
            attrs={"class": "date-type form-select form-select-sm", "id": "date"},
        ),
    )
